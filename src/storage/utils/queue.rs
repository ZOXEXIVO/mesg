...
impl QueueUtils {
    fn get_user_queues(db: &Database) -> impl Iterator<Item = String> {
        db.tree_names()
            .into_iter()
    ...
    pub fn get_ready_queues(db: &Database, base_queue_name: &str) -> Vec<String> {
        Self::get_user_queues(db)
            .filter(|db_queue| QueueNames::is_ready(db_queue, base_queue_name))
            .collect()
    }

    pub fn get_unack_queues(db: &Database) -> Vec<String> {
        Self::get_user_queues(db)
            .filter(|q| QueueNames::is_unack(q))
            .collect()
    }

    pub fn get_unack_order_queues(db: &Database) -> Vec<String> {
        Self::get_user_queues(db)
            .filter(|q| QueueNames::is_unack_order(q))
            .collect()
    }

    pub fn random_ready_queue_name(db: &Database, queue: &str) -> Option<String> {
